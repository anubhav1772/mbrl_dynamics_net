...
class eval_aliengo(object):
    def __init__(
        self, headless = False) -> None:
        super().__init__()

        label = "gait-conditioned-agility/2025-03-05/train"
        self.env = self.load_env(label, headless=headless)

    def load_env(self, label, headless=False):
        #dirs = glob.glob(f"runs/{label}/*")
        #logdir = sorted(dirs)[-1]
        logdir = '/home/anubhav1772/Documents/UniO4-Aliengo/runs/gait-conditioned-agility/2025-03-05/train/043940.990431'
    ...
    def play_aliengo(self, policy, mean, std, eval_episode_num = 5):
        all_returns = []
        for bppo in policy:
            pi_returns = []
            policy = bppo._policy
...
if __name__ == '__main__':
    ...
    play_aliengo(policy, headless=False)
